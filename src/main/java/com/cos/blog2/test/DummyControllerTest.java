...
public class DummyControllerTest {
	...
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { //json 데이터를 요청 => Java Object(MessageConverter의 Jackson 라이브러리가 변환해서 받아준다)
		...
		User user = userRepository.findById(id).orElseThrow(()->{ //DB user테이블에서 데이터를 가지고 온다. 만약 못가지고 온다면 orElseThrow문 실행(람다식 이용)
	...
	public User detail(@PathVariable int id) {
		...
		//그럼 return null이 리턴이 되잖아.. 그럼 프로그램에 문제가 있지 않겟니?
